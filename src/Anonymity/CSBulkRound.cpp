...
namespace Anonymity {
  ...
  QPair<QByteArray, bool> CSBulkRound::GetBlameData(int)
  {
    ...
    QByteArray msg(12, 0);
    Serialization::WriteUInt(_state->scheduler->my_idx, msg, 0);
  ...
  void CSBulkRound::ProcessDataShuffle()
  {
    ...
    for(int idx = 0; idx < count; idx++) {
      ...
      if(_state->shuffle_data == pair.second) {
        _state->scheduler->my_idx = _state->anonymous_keys.count();
      }
  ...
  void CSBulkRound::ProcessKeyShuffle()
  {
    ...
    _state->scheduler->my_idx = nks->GetKeyIndex();
    Q_ASSERT(_state->scheduler->my_idx > -1);

    _state->anonymous_keys = nks->GetKeys();
    Q_ASSERT(_state->scheduler->my_idx < _state->anonymous_keys.count());
  ...
  QByteArray CSBulkRound::GenerateCiphertext()
  {
    ...
    if(_state->scheduler->SlotOpen() && _state->close_slot) {
      ...
      qDebug() << "Writing ciphertext into my slot" << _state->scheduler->my_idx <<
        "starting at" << offset << "for" << my_msg.size() << "bytes.";

    } else if(CheckData()) {
      qDebug() << "Requesting to open my slot" << _state->scheduler->my_idx;
      xor_msg[_state->scheduler->my_idx / 8] = xor_msg[_state->scheduler->my_idx / 8] ^
        bit_masks[_state->scheduler->my_idx % 8];
  ...
  QByteArray CSBulkRound::GenerateSlotMessage()
  {
    ...
    int length = _state->next_msg.size() + SlotHeaderLength(_state->scheduler->my_idx);
...
#endif
    if(_state->accuse) {
      Serialization::WriteInt(SlotHeaderLength(_state->scheduler->my_idx), msg_p, 4);
...
//    int my_public_idx = GetGroup().GetIndex(GetLocalId());
    ...
    foreach(int owner, _state->scheduler->messages.keys()) {
      ...
      if(msg_pp.isEmpty()) {
        ...
        if(_state->scheduler->my_idx == owner) {
      ...
      if(hashalgo.ComputeHash(msg_p) != sig) {
        ...
        if(owner == _state->scheduler->my_idx && !_state->accuse) {
          /*
